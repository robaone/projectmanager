...
class HMAC_SHA1 extends OAuthSignatureMethod {
    ...
    protected String getSignature(String baseString) throws OAuthException {
        try {
            ...
            AppDatabase.writeLog("00054: HMAC_SHA1.getSignature() = "+signature);
            return signature;
    ...
    throws OAuthException {
        try {
        	AppDatabase.writeLog("00055: HMAC_SHA1.isValid(...)");
        	AppDatabase.writeLog("00056: baseString = "+baseString);
            byte[] expected = computeSignature(baseString);
            byte[] actual = decodeBase64(signature);
            AppDatabase.writeLog("00057: computed signature = "+base64Encode(expected));
            AppDatabase.writeLog("00058: actual signature   = "+signature);
    ...
    private byte[] computeSignature(String baseString)
        ...
        SecretKey key = null;
        AppDatabase.writeLog("00059: HMAC_SHA1.computeSignature(...)");
        AppDatabase.writeLog("00060: consumer secret = "+getConsumerSecret());
        synchronized (this) {
            if (this.key == null) {
