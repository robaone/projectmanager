				...
				if(result.getStatus() == ProjectConstants.OK){
					...
					if(token.equals("")){
						...
						if(w instanceof TasksList){
							...
							if(tokens.length > 1){
								if(tokens[1].startsWith(TasksList.TASK.ALERT.toString())){
									t.showTask(TasksList.TASK.ALERT);
								}else if(tokens[1].startsWith(TasksList.TASK.JOB.toString())){
									t.showTask(TasksList.TASK.JOB);
								}else if(tokens[1].startsWith(TasksList.TASK.NOTICE.toString())){
									t.showTask(TasksList.TASK.NOTICE);
								}else if(tokens[1].startsWith(TasksList.TASK.PROJECT.toString())){
