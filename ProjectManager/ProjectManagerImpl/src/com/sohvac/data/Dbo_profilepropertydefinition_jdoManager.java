...
public class Dbo_profilepropertydefinition_jdoManager {
  private Connection m_con;
  private final static String SELECT = "select ~ from #TABLE# where PropertyDefinitionID = ?";
  ...
  private final static String SEARCH = "select COUNT(#TABLE#.PROPERTYDEFINITIONID) from #TABLE# where #TABLE#.PROPERTYDEFINITIONID = ?";
  private final static String DELETE = "delete from #TABLE# where #TABLE#.PROPERTYDEFINITIONID = ?";
  private final static String IDENTITY = "PROPERTYDEFINITIONID";
  private final static String NEXT_SQL = "null";
  public final static String FIELDS = "#TABLE#.PROPERTYDEFINITIONID,#TABLE#.PORTALID,#TABLE#.MODULEDEFID,#TABLE#.DELETED,#TABLE#.DATATYPE,#TABLE#.DEFAULTVALUE,#TABLE#.PROPERTYCATEGORY,#TABLE#.PROPERTYNAME,#TABLE#.LENGTH,#TABLE#.REQUIRED,#TABLE#.VALIDATIONEXPRESSION,#TABLE#.VIEWORDER,#TABLE#.VISIBLE,#TABLE#.CREATEDBYUSERID,#TABLE#.CREATEDONDATE,#TABLE#.LASTMODIFIEDBYUSERID,#TABLE#.LASTMODIFIEDONDATE";
  private String TABLE = "DBO.PROFILEPROPERTYDEFINITION";
  public Dbo_profilepropertydefinition_jdoManager(Connection con){
    this.m_con = con;
  }
  ...
  public static Dbo_profilepropertydefinition_jdo bindDbo_profilepropertydefinition(ResultSet rs) throws SQLException{
Dbo_profilepropertydefinition_jdo retval = null;
    retval = Dbo_profilepropertydefinition_jdoManager.createObject(rs);
    return retval;
  }

  public Dbo_profilepropertydefinition_jdo getDbo_profilepropertydefinition(Integer propertydefinitionid){
    ...
    try{
      ...
      ps.setObject(1,propertydefinitionid);
      rs = ps.executeQuery();
      if(rs.next()){
        retval = Dbo_profilepropertydefinition_jdoManager.createObject(rs);
      }
...
* @throws SQLException
*/
  private static Dbo_profilepropertydefinition_jdo createObject(ResultSet rs) throws SQLException {
    Dbo_profilepropertydefinition_jdo retval = null;
    retval = new Dbo_profilepropertydefinition_jdo();
    /*
     *
  ...
  public void save(Dbo_profilepropertydefinition_jdo record) throws Exception {
    ...
    try{
      ...
      // Update
        ...
        String max_sql = "select max("+record.getIdentityName()+") from dbo.ProfilePropertyDefinition";
        PreparedStatement max_ps = con.prepareStatement(max_sql);
        ResultSet max_rs = max_ps.executeQuery();
        if(max_rs.next()){
          record.setPropertydefinitionid(max_rs.getInt(1));
        }
        max_rs.close();
        max_ps.close();
        con.commit();
	...
	protected void handleAfterInsert(Dbo_profilepropertydefinition_jdo record) {}
	protected void handleAfterUpdate(Dbo_profilepropertydefinition_jdo record) {}
	protected void handleBeforeUpdate(Dbo_profilepropertydefinition_jdo record) {}
  private void setAllClean(Dbo_profilepropertydefinition_jdo record){
    try{
      for(int i = 0; i < record.getFieldCount();i++){
         String fieldname = record.getField(i);
         Object[] val = record.getField(fieldname);
  ...
  public void delete(Dbo_profilepropertydefinition_jdo record) throws Exception {
    Connection con = this.getConnection();
...
public Dbo_profilepropertydefinition_jdo newDbo_profilepropertydefinition() {
Dbo_profilepropertydefinition_jdo retval = new Dbo_profilepropertydefinition_jdo();
 retval.setPropertydefinitionid(null);
 return retval;
}
