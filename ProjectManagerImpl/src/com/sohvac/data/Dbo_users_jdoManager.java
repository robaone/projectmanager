...
public class Dbo_users_jdoManager {
  private Connection m_con;
  private final static String SELECT = "select ~ from #TABLE# where UserID = ?";
  ...
  private final static String SEARCH = "select COUNT(#TABLE#.USERID) from #TABLE# where #TABLE#.USERID = ?";
  private final static String DELETE = "delete from #TABLE# where #TABLE#.USERID = ?";
  private final static String IDENTITY = "USERID";
  private final static String NEXT_SQL = "null";
  public final static String FIELDS = "#TABLE#.USERID,#TABLE#.USERNAME,#TABLE#.FIRSTNAME,#TABLE#.LASTNAME,#TABLE#.ISSUPERUSER,#TABLE#.AFFILIATEID,#TABLE#.EMAIL,#TABLE#.DISPLAYNAME,#TABLE#.UPDATEPASSWORD,#TABLE#.LASTIPADDRESS,#TABLE#.ISDELETED,#TABLE#.CREATEDBYUSERID,#TABLE#.CREATEDONDATE,#TABLE#.LASTMODIFIEDBYUSERID,#TABLE#.LASTMODIFIEDONDATE,#TABLE#.PASSWORD,#TABLE#.PHONENUMBER,#TABLE#.ADDRESS1,#TABLE#.ADDRESS2,#TABLE#.CITY,#TABLE#.STATE,#TABLE#.ZIP,#TABLE#.ZIP_EXT,#TABLE#.TYPE";
  private String TABLE = "DBO.USERS";
  public Dbo_users_jdoManager(Connection con){
    this.m_con = con;
  }
  ...
  public static Dbo_users_jdo bindDbo_users(ResultSet rs) throws SQLException{
Dbo_users_jdo retval = null;
    retval = Dbo_users_jdoManager.createObject(rs);
    return retval;
  }

  public Dbo_users_jdo getDbo_users(Integer userid){
...
* @throws SQLException
*/
  private static Dbo_users_jdo createObject(ResultSet rs) throws SQLException {
    Dbo_users_jdo retval = null;
    retval = new Dbo_users_jdo();
    /*
     *
  ...
  public void save(Dbo_users_jdo record) throws Exception {
    ...
    try{
      ...
      // Update
        ...
        String max_sql = "select max("+record.getIdentityName()+") from dbo.Users";
        PreparedStatement max_ps = con.prepareStatement(max_sql);
        ResultSet max_rs = max_ps.executeQuery();
        if(max_rs.next()){
          record.setUserid(max_rs.getInt(1));
        }
        max_rs.close();
        max_ps.close();
        con.commit();
	...
	protected void handleAfterInsert(Dbo_users_jdo record) {}
	protected void handleAfterUpdate(Dbo_users_jdo record) {}
	protected void handleBeforeUpdate(Dbo_users_jdo record) {}
  private void setAllClean(Dbo_users_jdo record){
    try{
      for(int i = 0; i < record.getFieldCount();i++){
         String fieldname = record.getField(i);
         Object[] val = record.getField(fieldname);
  ...
  public void delete(Dbo_users_jdo record) throws Exception {
    Connection con = this.getConnection();
...
public Dbo_users_jdo newDbo_users() {
Dbo_users_jdo retval = new Dbo_users_jdo();
 retval.setUserid(null);
 return retval;
}
