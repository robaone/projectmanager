...
public class History_jdo extends RO_JDO{
	...
	public void setObjectid(BigDecimal objectid){
		this.setField(OBJECTID,objectid);
	}
				...
				if(val[0] instanceof String){
					...
					ConnectionBlock block = new ConnectionBlock(){
						...
						protected void run() throws Exception {
							...
							this.getPreparedStatement().setBigDecimal(1, History_jdo.this.getId());
	...
	public String getText_value(final String table,HDBConnectionManager cman){
		...
		if(val != null && val[0] != null)try {
			...
			if(val[0] instanceof String){
				...
				ConfigManager.runConnectionBlock(block,cman);
				if(retval.size() > 0){
					return (String)retval.get(0);
				}else{
					return null;
