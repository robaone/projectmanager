...
public class Config_jdo extends RO_JDO{
	...
	public String getDescription(){
		...
		if(val != null && val[0] != null){
			if(val[0] instanceof java.sql.Clob){
				return ConfigManager.getClobString((java.sql.Clob)val[0]);
			}else if(val[0] instanceof org.hsqldb.types.ClobDataID){
				return ConfigManager.getClobString((org.hsqldb.types.ClobDataID)val[0]);
			}else{
				return (String)val[0];
			}
		}else{
	...
	public byte[] getBinary_value(){
		...
		if(val != null && val[0] != null){
			try{
				if(val[0] instanceof java.sql.Blob){
				...
				}else if(val[0] instanceof org.hsqldb.types.BlobDataID){
					return ConfigManager.getBlobBytes((org.hsqldb.types.BlobDataID)val[0]);
				}else{
					return (byte[])val[0];
				}
			}catch(Exception e){
