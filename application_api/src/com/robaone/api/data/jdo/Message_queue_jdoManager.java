...
public class Message_queue_jdoManager {
	...
	private final static String SELECT = "select ~ from #TABLE# where idmessage_queue = ?";
	...
	private final static String SEARCH = "select COUNT(#TABLE#.IDMESSAGE_QUEUE) from #TABLE# where #TABLE#.IDMESSAGE_QUEUE = ?";
	...
	public static Message_queue_jdo bindMessage_queue(ResultSet rs) throws SQLException{
		...
		try{
			...
			if(count > 0){
				...
				for(int i = 0;i < record.getFieldCount();i++){
					...
					insert_pre += this.TABLE+"."+fieldname;
					insert_post += "?";
				}
				insert_pre += ") values ";
				insert_post += ")";
				...
				if(max_rs.next()){
					record.setIdmessage_queue(max_rs.getInt(1));
				}
				max_rs.close();
				max_ps.close();
				con.commit();
	...
	public static JSONObject toJSONObject(Message_queue_jdo record) throws Exception {
	...
	public static void bindMessage_queueJSON(Message_queue_jdo record,String jsondata) throws Exception {
		JSONObject jo = new JSONObject(jsondata);
		Message_queue_jdoManager.bindMessage_queueJSON(record,jo);
	}
	public static void bindMessage_queueJSON(Message_queue_jdo record, JSONObject jo) throws Exception {
		...
		if(record != null && jo != null){
			...
			try{
				if(jo.isNull((String)keymap.get(Message_queue_jdo.HTML))){
					...
					record.setHtml(new Boolean(jo.getString((String)keymap.get(Message_queue_jdo.HTML))));
				}
			}catch(org.json.JSONException e){
			}
			try{
