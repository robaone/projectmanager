...
public class Dbo_userroles_jdoManager {
  private Connection m_con;
  private final static String SELECT = "select ~ from #TABLE# where UserRoleID = ?";
  ...
  private final static String SEARCH = "select COUNT(#TABLE#.USERROLEID) from #TABLE# where #TABLE#.USERROLEID = ?";
  private final static String DELETE = "delete from #TABLE# where #TABLE#.USERROLEID = ?";
  private final static String IDENTITY = "USERROLEID";
  private final static String NEXT_SQL = "null";
  public final static String FIELDS = "#TABLE#.USERROLEID,#TABLE#.USERID,#TABLE#.ROLEID,#TABLE#.EXPIRYDATE,#TABLE#.ISTRIALUSED,#TABLE#.EFFECTIVEDATE,#TABLE#.CREATEDBYUSERID,#TABLE#.CREATEDONDATE,#TABLE#.LASTMODIFIEDBYUSERID,#TABLE#.LASTMODIFIEDONDATE";
  private String TABLE = "DBO.USERROLES";
  public Dbo_userroles_jdoManager(Connection con){
    this.m_con = con;
  }
  ...
  public static Dbo_userroles_jdo bindDbo_userroles(ResultSet rs) throws SQLException{
Dbo_userroles_jdo retval = null;
    retval = Dbo_userroles_jdoManager.createObject(rs);
    return retval;
  }

  public Dbo_userroles_jdo getDbo_userroles(Integer userroleid){
...
* @throws SQLException
*/
  private static Dbo_userroles_jdo createObject(ResultSet rs) throws SQLException {
    Dbo_userroles_jdo retval = null;
    retval = new Dbo_userroles_jdo();
    /*
     *
  ...
  public void save(Dbo_userroles_jdo record) throws Exception {
    ...
    try{
      ...
      // Update
        ...
        String max_sql = "select max("+record.getIdentityName()+") from dbo.UserRoles";
        PreparedStatement max_ps = con.prepareStatement(max_sql);
        ResultSet max_rs = max_ps.executeQuery();
        if(max_rs.next()){
          record.setUserroleid(max_rs.getInt(1));
        }
        max_rs.close();
        max_ps.close();
        con.commit();
	...
	protected void handleAfterInsert(Dbo_userroles_jdo record) {}
	protected void handleAfterUpdate(Dbo_userroles_jdo record) {}
	protected void handleBeforeUpdate(Dbo_userroles_jdo record) {}
  private void setAllClean(Dbo_userroles_jdo record){
    try{
      for(int i = 0; i < record.getFieldCount();i++){
         String fieldname = record.getField(i);
         Object[] val = record.getField(fieldname);
  ...
  public void delete(Dbo_userroles_jdo record) throws Exception {
    Connection con = this.getConnection();
...
public Dbo_userroles_jdo newDbo_userroles() {
Dbo_userroles_jdo retval = new Dbo_userroles_jdo();
 retval.setUserroleid(null);
 return retval;
}
